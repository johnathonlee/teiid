...
public class TestJoinOptimization {
    ...
    @Test public void testJoinNullHandling() throws Exception {
    	...
    	plan = TestOptimizer.helpPlan(sql, RealMetadataFactory.exampleBQTCached(), new String[] {"SELECT bqt1.smalla.intkey FROM bqt1.smalla ORDER BY bqt1.smalla.intkey NULLS FIRST", "SELECT bqt2.smalla.intkey FROM bqt2.smalla ORDER BY bqt2.smalla.intkey"}, capFinder, ComparisonMode.EXACT_COMMAND_STRING); //$NON-NLS-1$ //$NON-NLS-2$
